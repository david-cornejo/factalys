...
} = require('../middlewares/validations/serviciosValidation');
const { checkRoles } = require('./../middlewares/authHandler');
const serviciosServicios = require('../service/serviciosServices');
...
router.post(
  ...
  checkSchema(createServicioSchema),
  validateRequest,
  async (req, res) => {
    const body = req.body;
...
router.patch(
  ...
  passport.authenticate('jwt', { session: false }),
  checkRoles('super', 'corporativo'),
  checkSchema(updateServicioSchema),
  validateRequest,
  async (req, res, next) => {
    try {
      const { id } = req.params;
...
router.delete(
  ...
  passport.authenticate('jwt', { session: false }),
  checkRoles('super'),
  checkSchema(deleteServicioSchema),
  validateRequest,
  async (req, res) => {
    try {
      const { id } = req.params;
